...
SoybeanAdminは現在、全自動のプロキシ設定を完全に組み込んでいます。詳細については[プロキシ](/jp/guide/request/proxy.html)をご覧ください。

> ローカル開発環境では、デフォルトでローカルプロキシが有効です。

**本番環境でのクロスオリジン**

プロジェクトを本番環境にデプロイした後、一般的には Nginx を使ってリクエストをバックエンドサーバーに転送します。詳細については、AIにリクエストアドレスを提供し、具体的な設定手順を尋ねることができます。

> バックエンドサービスが CORS の通過を許可していれば、フロントエンドサービスでは追加の設定は必要ありません
